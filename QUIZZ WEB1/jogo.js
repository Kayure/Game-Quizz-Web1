...
let categoryGame = function (id, name) {
  ...
  let game = function (startAnswerLater, currentQuestion, difficulty, score, stop, errors, questionsAnswered) {
  ...
  function questions() {
    axios.get(`https://opentdb.com/api.php?amount=10&category=${categoryQuestions.id}&difficulty=${currentGame.difficulty}`).then(function (response) {
  ...
  //FUNÇÃO PARA DEFINICIR CATEGORIA E DIFICULDADE 
  ...
  //FUNÇÃO PARA DEFINICIR A QUESTÃO
            ...
            if (currentQuestion.answerLater !== undefined) {
              ...
              for (let index = 0; index < possibilities.length; index++) {
                ...
                if (conditions[index] === true) {
                  ...
                  button.addEventListener('click', function (e) {
                    loadStoredQuestion();
                    //COLOCAR NA FUNCAO  OS PONTOS A SEREM CONTABILIZADOS PARA AS PERGUNTAS 
                    document.querySelector('.new-question').classList.add('hidden');
                    document.querySelector('.buttons-later').classList.remove('hidden');
                    startIntervalAnswerLater();
